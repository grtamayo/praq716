/*
	Filename:     praq715.c
	Description:  PPP style or simply LZP. Outputs (1 bit for LZP matched length + matched length)
	              or (1 bit for mismatched byte + mismatched byte).
	Written by:   Gerald R. Tamayo, 7/25/2023
*/
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
void usage( void )
{
	fprintf(stderr, "\n Usage: praq715 c[N]|d infile outfile\n"
...
void compress_LZP( unsigned char w[], unsigned char p[] )
{
	int blen=0, n, nread, prev=0;  /* prev = context hash */
	unsigned char c;
...
void decompress_LZP( unsigned char w[] )
{
	int blen, c, prev = 0;  /* prev = context hash */
