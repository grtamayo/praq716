/*
	Filename:     praq718.c
	Description:  PPP style or simply LZP. Outputs (1 bit + matched byte) or (1 bit + mismatched length + mismatched bytes).
...
void usage( void )
{
	fprintf(stderr, "\n Usage: praq718 c[N]|d infile outfile\n"
...
void compress_LZP( unsigned char w[], unsigned char p[] )
{
	int i, clen=0, n, nread, prev=0;  /* prev = context hash */
...
void decompress_LZP( unsigned char w[] )
{
	int i, c, clen, prev = 0;  /* prev = context hash */
